...
namespace QuanLyCaPhe.DAO
{
    public class WorkScheduleDAO
    {
        ...
        public bool UpdateSchedule(int id, DateTime date, TimeSpan start, TimeSpan end)
        {
            string query = "UPDATE WorkSchedules SET WorkDate = @date, StartTime = @start, EndTime = @end WHERE Id = @id";
            return DBHelper.ExecuteNonQuery(query, new SqlParameter[] {
                new SqlParameter("@date", date),
                new SqlParameter("@start", start),
                new SqlParameter("@end", end),
